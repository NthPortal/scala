...
class SetFromMap[A](private val underlying: Map[A, Unit])
  ...
  def incl(elem: A): SetFromMap[A] = sfm(underlying.updated(elem, ()))

  def excl(elem: A): SetFromMap[A] = sfm(underlying.removed(elem))
...
object SetFromMap {
  ...
  private class WrapperBuilder[A](b: mutable.Builder[(A, Unit), Map[A, Unit]]) extends mutable.Builder[A, SetFromMap[A]] {
